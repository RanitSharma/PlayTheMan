...
export class PokerEngine {
  ...
  static evaluateHand(cards: Card[]): Evaluation {
    ...
    if (straightFlushHigh !== -1) return { score: 9000000 + straightFlushHigh, label: straightFlushHigh === 14 ? "Royal Flush" : "Straight Flush", bestFive: sfCards };
    if (quads.length) return { score: 8000000 + this.RANK_VALUE[quads[0][0]], label: "Four of a Kind", bestFive: [...quads[0][1], sorted.find(c => c.rank !== quads[0][0])!] };
    if (trips.length && (trips.length > 1 || pairs.length > 0)) {
      ...
      return { score: 7000000 + this.RANK_VALUE[mainTrips[0]] * 100 + this.RANK_VALUE[secondSet[0].rank], label: "Full House", bestFive: [...mainTrips[1], ...secondSet] };
    }
    if (flushSuitCards) return { score: 6000000 + this.RANK_VALUE[flushSuitCards[0].rank], label: "Flush", bestFive: flushSuitCards.slice(0, 5) };
    if (straightHigh !== -1) {
      let sCards: Card[] = [];
      if (straightHigh === 5 && uniqueRanks.includes(14)) {
        const ranks = [5, 4, 3, 2, 14];
        sCards = ranks.map(r => sorted.find(c => this.RANK_VALUE[c.rank] === r)!);
      } else {
    ...
    if (trips.length) return { score: 4000000 + this.RANK_VALUE[trips[0][0]], label: "Three of a Kind", bestFive: [...trips[0][1], ...sorted.filter(c => c.rank !== trips[0][0]).slice(0, 2)] };
    if (pairs.length >= 2) return { score: 3000000 + this.RANK_VALUE[pairs[0][0]] * 100 + this.RANK_VALUE[pairs[1][0]], label: "Two Pair", bestFive: [...pairs[0][1], ...pairs[1][1], sorted.find(c => c.rank !== pairs[0][0] && c.rank !== pairs[1][0])!] };
    if (pairs.length === 1) return { score: 2000000 + this.RANK_VALUE[pairs[0][0]], label: "One Pair", bestFive: [...pairs[0][1], ...sorted.filter(c => c.rank !== pairs[0][0]).slice(0, 3)] };
    return { score: 1000000 + this.RANK_VALUE[sorted[0].rank], label: "High Card", bestFive: sorted.slice(0, 5) };
  }
  ...
  static calculatePots(players: Player[]): Pot[] {
    ...
    const allInLevels = contributors.filter(p => p.isAllIn).map(p => p.betThisRound);
    const contributionLevels = [...new Set([...allInLevels, ...contributors.map(p => p.betThisRound)])].sort((a, b) => a - b);
    ...
    for (const currentLevel of contributionLevels) {
      ...
      const matchedThisSlice = contributors.filter(p => p.betThisRound >= currentLevel);
      const eligible = matchedThisSlice.filter(p => !p.isFolded).map(p => p.id);
      
      if (matchedThisSlice.length > 0) {
        pots.push({ 
          amount: parseFloat((increment * matchedThisSlice.length).toFixed(2)), 
